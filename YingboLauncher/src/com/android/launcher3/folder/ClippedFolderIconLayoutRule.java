...
public class ClippedFolderIconLayoutRule implements FolderIcon.PreviewLayoutRule {
    ...
    /* 20190107 modify by SIM cuixiangwen begin */
    private static final int NUM_ITEMS_IN_PREVIEW = 4;
    private static final float PERSPECTIVE_SCALE_FACTOR = 0.45f;
    private static final float PERSPECTIVE_SHIFT_FACTOR = 0.24f;
    /* 20180702 modify by SIM cuixiangwen end */
    ...
    public PreviewItemDrawingParams computePreviewItemDrawingParams(int index, int curNumItems,
        ...
        /* 20190107 modify by SIM cuixiangwen begin */
        String folderPreviewValues = System.getProperty("folder_preview","0");
        switch(folderPreviewValues){
        /* 20180702 modify by SIM cuixiangwen end */
	...
	float r = (index * 1.0f) / (NUM_ITEMS_IN_PREVIEW - 1);
        float scale = (1 - PERSPECTIVE_SCALE_FACTOR * (1 - r))*0.85f;

        float offset = (1 - r) * 37f;
        float scaledSize = scale * 78;
        float scaleOffsetCorrection = (1 - scale) * 78;  
  
       // We want to imagine our coordinates from the bottom left, growing up and to the  
       // right. This is natural for the x-axis, but for the y-axis, we have to invert things.  
        float transY1 = 110 - (offset + scaledSize + scaleOffsetCorrection);
        float transX1 = offset + scaleOffsetCorrection - 13;
        float totalScale1 = 0.75f * scale;
        final int overlayAlpha1 = (int) (80 * (1 - r));  

       if (params == null) {  
           params = new PreviewItemDrawingParams(transX1, transY1, totalScale1, overlayAlpha1);  
       } else {  
           params.transX = transX1;  
           params.transY = transY1;  
           params.scale = totalScale1;  
           params.overlayAlpha = overlayAlpha1;  
       }
//  	Log.d("cuixiangwen","--transX is :" + transX1 + "--transY is :" + transY1 + "--totalScale is :" + totalScale1 + "--overlayAlpha1 is :" + overlayAlpha1);
  	break;
       
	//线性视图
	case "2":
	index = NUM_ITEMS_IN_PREVIEW - index - 1; 
	float r2 = (index * 1.0f) / (NUM_ITEMS_IN_PREVIEW - 1);
        float scale2 = (1 - PERSPECTIVE_SCALE_FACTOR * (1 - r2))*0.9f;

        float offset2 = (1 - r2) * 37f;
        float scaledSize2 = scale2 * 78;
        float scaleOffsetCorrection2 = (1 - scale2) * 78;  
  
        // We want to imagine our coordinates from the bottom left, growing up and to the  
        // right. This is natural for the x-axis, but for the y-axis, we have to invert things.  
        float transY2 = 106 - (offset2 + scaledSize2 + scaleOffsetCorrection2);
        float transX2 = offset2/2 + 14f;
        float totalScale2 = 0.75f * scale2;
        final int overlayAlpha2 = (int) (80 * (1 - r2));  

       if (params == null) {  
           params = new PreviewItemDrawingParams(transX2, transY2, totalScale2, overlayAlpha2);  
       } else {  
           params.transX = transX2;  
           params.transY = transY2;  
           params.scale = totalScale2;  
           params.overlayAlpha = overlayAlpha2;  
       		}
       	break;
       	default : break;
       	}	
