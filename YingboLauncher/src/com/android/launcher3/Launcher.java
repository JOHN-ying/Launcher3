...
//        -----------------------------------------------------------------------------
    ...
    protected void onResume() {
        ...
        //add by SIM cuixiangwen 2019/1/3 start
        if(Utilities.isWallpaperScrollsEnabled(getApplicationContext())) {
		System.setProperty(WALLPAPER_SCROLLS_WITH_MAIN_SCREEN,"1");
	}else{
		System.setProperty(WALLPAPER_SCROLLS_WITH_MAIN_SCREEN,"0");
	}       
        if(Utilities.isNotificationBarDisplayEnabled(getApplicationContext())){
             hideStatusBarView();
        }
        if(Utilities.isNotificationBarTransparencyEnabled(getApplicationContext())){
            transparentStatusBarView();        	
        }else{
            untransparentStatusBarView();
        }
	folderPreview(getApplicationContext());
        //add by SIM cuixiangwen 2019/1/3 end                  
        if (mLauncherCallbacks != null) {
...
//                | View.SYSTEM_UI_FLAG_HIDE_NAVIGATION
    ...
    protected void untransparentStatusBarView() {
...
//             window.setNavigationBarColor(Color.TRANSPARENT);
    ...
    protected void transparentStatusBarView() {
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
             Window window = getWindow();
...
//             window.setNavigationBarColor(Color.TRANSPARENT);
         ...
         protected void folderPreview(Context context){
		String folderPreview = Utilities.folderPreviewValues(context);
		switch(folderPreview){
			case "grid":	
			System.setProperty("folder_preview","0");		
			break;
			case "stack":	
			System.setProperty("folder_preview","1");
			break;
			case "linear":	
			System.setProperty("folder_preview","2");
			break;
			default: break;
		} 
	}
...
//add by lkk
    ...
    public void bindScreens(ArrayList<Long> orderedScreenIds) {
        ...
        boolean qsbOnFisrtScreem = Utilities.isAllowSearchWidgetPrefEnabled(this);
        if (qsbOnFisrtScreem &&
                orderedScreenIds.indexOf(Workspace.FIRST_SCREEN_ID) != 0) {
            orderedScreenIds.remove(Workspace.FIRST_SCREEN_ID);
            orderedScreenIds.add(0, Workspace.FIRST_SCREEN_ID);
            LauncherModel.updateWorkspaceScreenOrder(this, orderedScreenIds);
        } else if (!qsbOnFisrtScreem && orderedScreenIds.isEmpty()) {
    ...
    private void bindAddScreens(ArrayList<Long> orderedScreenIds) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!Utilities.isAllowSearchWidgetPrefEnabled(this) || screenId != Workspace.FIRST_SCREEN_ID) {
    ...
    private class RotationPrefChangeHandler implements OnSharedPreferenceChangeListener {
        ...
        public void onSharedPreferenceChanged(
			...
			if (Utilities.ALLOW_SEARCH_WIDGET_KEY.equals(key)) {
